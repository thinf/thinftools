...
%   
%   The median filter is only sensibly defined with a ``boxcar'' filter.
...
%         thinf:
%         STR = 'const' for constant (end point) value
%         STR = 'trend' for extrapolating linear trend
%
...
switch str
    ...
    case 'con' % thinf: pad constant (end point) value
        data_extended(ia,:)=data(1,:);
        data_extended(ib,:)=data(N,:);
    case 'tre' % thinf: continue linear trend
        for rr = 1:size(data,2)
            pa = polyfit(ia,data(ia,rr),1);
            pb = polyfit(N-n+ia,data(N-n+ia,rr),1);
            data_extended(ia,rr)=polyval(pa,ia-n);
            data_extended(ib,rr)=polyval(pb,N-n+ia+n);
            clear rr pa pb
        end
    case 'per'
